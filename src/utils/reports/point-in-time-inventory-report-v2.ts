...
export interface IPointInTimeInventoryReportV2FormFilters extends ILicenseFormFilters {
  targetDate: string;
...
export const pointInTimeInventoryV2FormFiltersFactory: () => IPointInTimeInventoryReportV2FormFilters =
  () => ({
...
export function addPointInTimeInventoryV2Report({
  reportConfig,
  pointInTimeInventoryV2FormFilters,
}: {
  reportConfig: IReportConfig;
  pointInTimeInventoryV2FormFilters: IPointInTimeInventoryReportV2FormFilters;
}) {
  reportConfig[ReportType.POINT_IN_TIME_INVENTORY_V2] = {
    targetDate: pointInTimeInventoryV2FormFilters.targetDate,
    inventoryStrategy: pointInTimeInventoryV2FormFilters.inventoryStrategy,
    useRestrictedWindowOptimization:
      pointInTimeInventoryV2FormFilters.useRestrictedWindowOptimization,
    restrictedWindowDays: pointInTimeInventoryV2FormFilters.restrictedWindowDays,
    showDebugColumns: pointInTimeInventoryV2FormFilters.showDebugColumns,
    ...extractLicenseFields(pointInTimeInventoryV2FormFilters),
    fields: null,
  };
}

export async function maybeLoadPointInTimeInventoryReportV2Data({
  ctx,
  reportData,
  reportConfig,
}: {
  ...
  const pointInTimeInventoryV2ReportConfig = reportConfig[ReportType.POINT_IN_TIME_INVENTORY_V2];

  if (!pointInTimeInventoryV2ReportConfig) {
    return;
  }

  if (!pointInTimeInventoryV2ReportConfig?.targetDate) {
    throw new Error("Must provide target date");
  }

  if (pointInTimeInventoryV2ReportConfig?.targetDate > todayIsodate()) {
    throw new Error("Cannot select a date in the future");
  }

  for (const license of pointInTimeInventoryV2ReportConfig.licenses) {
  ...
  const days = pointInTimeInventoryV2ReportConfig.useRestrictedWindowOptimization ? 365 : 5000;
  const minDate = getIsoDateFromOffset(-days, pointInTimeInventoryV2ReportConfig.targetDate);
  const maxDate = getIsoDateFromOffset(days, pointInTimeInventoryV2ReportConfig.targetDate);
  ...
  for (const [label, metadata] of packageMetadataMap.entries()) {
    ...
    if (
      ...
      metadata.packagedDate > pointInTimeInventoryV2ReportConfig.targetDate
    ) {
      continue;
    }
    ...
    if (
      ...
      metadata.archivedDate < pointInTimeInventoryV2ReportConfig.targetDate
    ) {
      continue;
    }
    ...
    if (
      ...
      metadata.finishedDate < pointInTimeInventoryV2ReportConfig.targetDate
    ) {
      continue;
    }
    ...
    if (
      ...
      metadata.tagUsedDate > pointInTimeInventoryV2ReportConfig.targetDate
    ) {
      continue;
    }
    ...
    if (
      ...
      metadata.receivedDate > pointInTimeInventoryV2ReportConfig.targetDate
    ) {
      continue;
    }
    ...
    try {
      const isCustodied = isCustodiedDatetimeOrError({
        ...
        targetDatetime: pointInTimeInventoryV2ReportConfig.targetDate,
      });
  ...
  reportData[ReportType.POINT_IN_TIME_INVENTORY_V2] = {
...
export function extractPointInTimeInventoryData({
  ...
  if (reportConfig[ReportType.POINT_IN_TIME_INVENTORY_V2]!.showDebugColumns) {
    headers.push(
      "Debug Message",
      "Incoming Manifests",
      "Outgoing Manifests",
  ...
  const pairs = reportData[ReportType.POINT_IN_TIME_INVENTORY_V2]!.packageMetadataPairs;
