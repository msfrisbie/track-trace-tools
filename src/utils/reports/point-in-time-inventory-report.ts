...
export async function maybeLoadPointInTimeInventoryReportData({
  ...
  for (const incomingTransfer of filteredIncomingTransfers) {
    promises.push(
      primaryDataLoader.destinationPackages(incomingTransfer.DeliveryId).then((packages) => {
  ...
  for (const outgoingTransfer of filteredOutgoingTransfers) {
    promises.push(
      primaryDataLoader.transferDestinations(outgoingTransfer.Id).then((destinations) => {
