...
export async function maybeLoadCogsTrackerReportData({
  ...
  const historyPromises: Promise<any>[] = dateFilteredPackages.map((pkg) => primaryDataLoader.packageHistoryByPackageId(pkg.Id).then((history) => {
    pkg.history = history;
  }));
  ...
  function cogsTrackerInputRowFactory(pkg: IIndexedPackageData): any[] {
    ...
    const testMaterialWeightSum = testLabels
      .map(
        (testLabel) => (childTagQuantityUnitItemSets.find((x) => x[0] === testLabel) ?? [null, 0, null])[1],
      )
      .reduce((a, b) => a + b, 0);
  ...
  packagedGoodsCogsPackages.map((pkg) => packagedGoodsCogsMatrix.push(cogsTrackerOutputRowFactory(pkg)));
