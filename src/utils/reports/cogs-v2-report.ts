...
export function addCogsV2Report({
  ...
  const { licenses } = cogsV2FormFilters;
  ...
  if (cachedValue) {
    ...
    for (const transfer of richOutgoingTransfers) {
      richOutgoingTransferDestinationRequests.push(
        getDataLoaderByLicense(transfer.LicenseNumber).then((dataLoader) => dataLoader.transferDestinations(transfer.Id).then((destinations) => {
    ...
    richOutgoingTransfers = richOutgoingTransfers
      .map((x) => {
        x.outgoingDestinations = (x.outgoingDestinations || []).filter(
          (y) => y.ShipmentTypeName.includes('Wholesale')
    ...
    for (const transfer of richOutgoingTransfers) {
      for (const destination of transfer.outgoingDestinations || ([] as IRichDestinationData[])) {
        packageRequests.push(
          getDataLoaderByLicense(transfer.LicenseNumber).then((dataLoader) => dataLoader.destinationPackages(destination.Id).then((destinationPackages) => {
...
export async function updateCogsV2MasterCostSheet({
  ...
  try {
    ...
    const spreadsheetId = extractSheetIdOrError(store.state.client.values.MASTER_PB_COST_SHEET_URL);
...
export function extractMultiplierFromItemNamesOrError({
  ...
  // Test split match
  const [parentFirstClause, parentSecondClause, ...parentRemainder] = parentItemName.split(parentReadableQuantity);
  const [childFirstClause, childSecondClause, ...childRemainder] = childItemName.split(childReadableQuantity);
  ...
  for (const transfer of richOutgoingTransfers) {
    ...
    for (const destination of transfer.outgoingDestinations || []) {
      for (const manifestPkg of destination.packages || []) {
        ...
        const sourcePackage: IIndexedPackageData = scopedAncestorPackageMap.get(sourcePackageLabel)!;
        ...
        if (isEligibleForItemOptimization) {
          ...
          try {
            ...
            while (connectedMultiplierBuffer.length > 0) {
              ...
              const newMultiplier = currentMultiplier * (childPackageQuantity[1] / initialProductionBatchQuantity[0]);
            ...
            if (status === 'INITIAL') {
              costEquation = `=${finalBuffer
                .map(([pkg, multiplier]) => {
                  const initialProductionBatchQuantity = extractInitialPackageQuantityAndUnitFromHistoryOrError(pkg.history!);
        ...
        const sourceProductionBatch: string = sourcePackage.ProductionBatchNumber.length > 0
          ? sourcePackage.ProductionBatchNumber
          : sourcePackage.SourceProductionBatchNumbers;
