...
export default Vue.extend({
    ...
    buildCsvFiles(): ICsvFile[] {
      ...
      try {
        const csvData = buildCsvDataOrError([
          {
            isVector: true,
            data: this.transferPackages.map((packageData: IUnionIndexedPackageData) => getLabelOrError(packageData)),
          },
          {
            isVector: false,
  ...
  async mounted() {
    ...
    try {
      ...
      this.$data.defaultPhoneNumberForQuestions = await dynamicConstsManager.defaultPhoneNumberForQuestions();
