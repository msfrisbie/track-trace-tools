          ...
          <template v-if="!showTagPicker">
            ...
            <template v-if="showChildCountEntry">
              <div class="flex flex-col items-stretch p-4 space-y-4">
                <mother-plant-picker
                  :selectedMothers="selectedPlantBatches"
    ...
    buildCsvFiles(): ICsvFile[] {
      ...
      try {
        ...
        return buildNamedCsvFileData(
          csvData,
          `Taking ${this.totalChildCountImpl()} ${this.$data.plantBatchType.Name.toLocaleLowerCase() +
            "s"} from ${this.$data.selectedPlantBatches.length} mothers`
        );
      } catch (e) {
        console.error(e);
        return [];
    ...
    allPlantBatchesHaveValidChildCountImpl() {
      const result = arrayIsValid(this.$data.childMatrix, {
        collectionValidators: [
          {
            fn: (rows: any[]): boolean => {
              return rows.length === this.$data.selectedPlantBatches.length;
            },
  ...
  watch: {
    ...
    childMatrix: {
      ...
      handler(newValue: number[][], oldValue) {
        ...
        for (let [plantBatch, counts] of zipped) {
          // @ts-ignore
