  ...
  methods: {
    ...
    submit() {
      ...
      const flattenedPlantBatches = this.$data.childPackageData
        .map((x: IIntermediateCreatePackageFromMotherPlantBatchData) => x.counts.map(() => x.plantBatch))
        .flat();
    ...
    buildCsvFiles(): ICsvFile[] {
      ...
      const flattenedPlantBatchLabels = this.$data.childPackageData
        .map((x: IIntermediateCreatePackageFromMotherPlantBatchData) => x.counts.map(() => x.plantBatch.Name))
        .flat();
      const flattenedPlantCounts = this.$data.childPackageData
        .map((x: IIntermediateCreatePackageFromMotherPlantBatchData) => x.counts)
        .flat();
    ...
    totalChildCountImpl(): number {
      return sum(
        this.$data.childPackageData.map((x: IIntermediateCreatePackageFromMotherPlantBatchData) => sum(x.counts)),
      );
    },
  },
  computed: {
