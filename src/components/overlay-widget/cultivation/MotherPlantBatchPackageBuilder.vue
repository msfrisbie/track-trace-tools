        ...
        <div class="grid grid-cols-2 grid-rows-2 gap-8 h-full" style="grid-template-rows: 1fr auto">
          <template v-if="!showTagPicker">
              ...
              <template v-if="item">
                ...
                <template v-else>
                  <b-button class="opacity-40" variant="light"
                    @click="showHiddenDetailFields = true">ADVANCED</b-button>
                </template>
              </template>
            </div>
            ...
            <template v-if="showChildCountEntry">
              <div class="flex flex-col items-stretch p-4 space-y-4">
                <mother-plant-picker :selectedMothers="selectedPlantBatches" :plantBatchType="plantBatchType"
          ...
          <template v-if="showTagPicker">
            ...
            <tag-picker :tagTypeNames="['CannabisPackage', 'MedicalPackage', 'Cannabis Package', 'Medical Package']"
              :tagCount="newPackageCount" :selectedTags.sync="packageTags" />
        ...
        <template v-if="!errorMessage">
          <div class="flex flex-col items-center">
            <div class="flex flex-col space-y-4 text-xl" style="width: 600px">
              ...
              <div>
                Creates
                <span class="font-bold ttt-purple">{{ newPackageCount }} {{ item.Name }} packages.</span>
              </div>
              ...
              <b-button class="w-full" variant="success" size="md" @click="submit()">PACKAGE {{ totalChildCount }}
...
import {
  ...
  ICsvFile,
  IIntermediateCreatePackageFromMotherPlantBatchData,
...
export default Vue.extend({
  ...
  methods: {
    ...
    submit() {
      const rows: IMetrcCreatePlantBatchPackagesFromMotherPlantBatchPayload[] = [];

      const flattenedPlantBatches = this.$data.childPackageData
        .map((x: IIntermediateCreatePackageFromMotherPlantBatchData) =>
          x.counts.map(() => x.plantBatch))
        .flat();
      const flattenedPlantCounts = this.$data.childPackageData
        .map((x: IIntermediateCreatePackageFromMotherPlantBatchData) => x.counts)
        .flat();

      // @ts-ignore
      const zipped: [ITagData[], IPlantBatchData[], number[]] = safeZip(
        this.$data.packageTags,
        flattenedPlantBatches,
        flattenedPlantCounts,
      );

      for (const el of zipped) {
        const tag = el[0];
        const motherPlantBatch = el[1];
        const childCount = el[2];

        const row: IMetrcCreatePlantBatchPackagesFromMotherPlantBatchPayload = {
          ItemId: this.$data.item.Id.toString(),
          Note: this.$data.note,
          ActualDate: submitDateFromIsodate(this.$data.actualIsodate),
          IsFromMotherPlant: 'true',
          PlantBatchId: motherPlantBatch.Id.toString(),
          Count: childCount.toString(),
      ...
      builderManager.submitProject(
        ...
        {
          motherPlantBatchCount: this.$data.selectedPlantBatches.length,
    ...
    buildCsvFiles(): ICsvFile[] {
      ...
      const flattenedPlantBatchLabels = this.$data.childPackageData
        .map((x: IIntermediateCreatePackageFromMotherPlantBatchData) =>
          x.counts.map(() => x.plantBatch.Name))
        .flat();
      const flattenedPlantCounts = this.$data.childPackageData
        .map((x: IIntermediateCreatePackageFromMotherPlantBatchData) => x.counts)
        .flat();
      ...
      try {
        const csvData = buildCsvDataOrError([
          {
            isVector: true,
            data: flattenedPlantBatchLabels,
          },
          {
            isVector: false,
        ...
        return buildNamedCsvFileData(
          csvData,
          `Taking ${this.totalChildCountImpl()} ${`${this.$data.plantBatchType.Name.toLocaleLowerCase()}s`} from ${this.$data.selectedPlantBatches.length
          } mothers`,
        );
      } catch (e) {
        console.error(e);
    ...
    allPlantBatchesHaveValidChildCountImpl() {
    ...
    newPackageCountImpl(): number {
      return sum(
        this.$data.childPackageData.map(
          (x: IIntermediateCreatePackageFromMotherPlantBatchData) => x.counts.length,
        ),
      );
    },
    totalChildCountImpl(): number {
      return sum(
        this.$data.childPackageData.map((x: IIntermediateCreatePackageFromMotherPlantBatchData) =>
          sum(x.counts)),
      );
    },
  },
  computed: {
    ...
    allPlantBatchesHaveValidChildCount(): boolean {
      return this.allPlantBatchesHaveValidChildCountImpl();
    },
    ...
    errors(): IBuilderComponentError[] {
      ...
      if (emptyChildLists > 0 && emptyChildLists < this.childMatrix.length) {
        if (!(this as any).allPlantBatchesHaveValidChildCountImpl()) {
          errors.push({
            tags: ['page2'],
            message:
  ...
  watch: {
    ...
    childMatrix: {
      ...
      handler(newValue: number[][], oldValue) {
        ...
        if (this.$data.selectedPlantBatches.length !== newValue.length) {
          console.error('length mismatch');
          return;
        }

        const zipped = safeZip(this.$data.selectedPlantBatches, newValue);

        for (const [plantBatch, counts] of zipped) {
          // @ts-ignore
  ...
  data() {
    return {
      ...
      steps: [
        {
          stepText: 'Select mother plant batches',
        },
        {
