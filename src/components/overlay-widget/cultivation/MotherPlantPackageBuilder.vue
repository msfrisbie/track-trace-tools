    ...
    <div class="col-span-3 h-full">
      <template v-if="activeStepIndex === 0">
        <div class="grid grid-cols-2 grid-rows-2 gap-8 h-full" style="grid-template-rows: 1fr auto">
          <plant-picker :enableVegetative="true" class="col-span-2" :builderType="builderType"
          ...
          <template v-if="!showTagPicker">
            ...
            <template v-if="showChildCountEntry">
              <div class="flex flex-col items-stretch p-4 space-y-4">
                <mother-plant-picker :selectedMothers="selectedPlants" :plantBatchType="plantBatchType"
    ...
    buildCsvFiles(): ICsvFile[] {
      ...
      try {
        ...
        return buildNamedCsvFileData(
          csvData,
          `Taking ${this.totalChildCountImpl()} ${`${this.$data.plantBatchType.Name.toLocaleLowerCase()}s`} from ${this.$data.selectedPlants.length
          } mothers`,
        );
      } catch (e) {
        console.error(e);
