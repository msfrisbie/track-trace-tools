...
export default Vue.extend({
  ...
  methods: {
    ...
    filterSelectedByPastedTags() {
      this.$data.selectedPackagesMirror = this.$data.sourcePackages.filter((x: IPackageData) => this.$data.pastedTags.includes(x.Label));
    },
    selectAll() {
      this.$data.selectedPackagesMirror = this.$data.sourcePackages;
    },
    async loadPackages() {
      ...
      try {
        ...
        if (this.$data.lockUuid === lock) {
          ...
          this.$data.sourcePackages = filteredPackages.sort((a: IPackageData, b: IPackageData) => (a.Label > b.Label ? 1 : -1));
