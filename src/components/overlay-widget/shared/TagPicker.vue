...
export default Vue.extend({
  ...
  methods: {
    ...
    filterSelectedByPastedTags() {
      this.$data.selectedTagsMirror = this.$data.sourceTags.filter((x: ITagData) => this.$data.pastedTags.includes(x.Label));
    },
    selectAll() {
      this.$data.selectedTagsMirror = this.$data.sourceTags;
    },
    async loadTags() {
      ...
      try {
        ...
        const tags = (await primaryDataLoader.availableTags({ useCache: false })).filter((tag) => this.$props.tagTypeNames.includes(tag.TagTypeName));

        // If there was a subsequent load, don't overwrite
        if (this.$data.lockUuid === lock) {
          this.$data.sourceTags = tags.sort((a: ITagData, b: ITagData) => (a.Label > b.Label ? 1 : -1));
