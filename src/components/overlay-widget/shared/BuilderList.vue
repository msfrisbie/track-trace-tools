      ...
      <div v-for="option of options" :key="option.text"
        ...
        <div class="w-full">
          <b-button
            ...
            }" :disabled="!option.enabled" @click.stop.prevent="selectBuilderType(option)">
              ...
              <div class="aspect-square grid place-items-center">
                <template v-if="option.isPlus">
                  ...
                  <b-badge style="padding-top: 0.3rem; margin-top: 0.1rem; line-height: initial"
                    variant="light">T3+</b-badge></template>
            ...
            <template v-if="!option.enabled">
              <template v-if="option.isPlus && !clientState.t3plus">
                <span class="text-xs flex flex-row items-center justify-center">
                  ...
                  <b-button size="sm" variant="link" class="underline"
                    @click.stop.prevent="open('/plus')">Learn&nbsp;more</b-button>
                </span>
              </template>
              <template v-if="!option.isPlus">
                <span class="text-xs flex flex-row items-center justify-center">
                  {{ notAvailableMessage }}
                  <b-button variant="link" size="sm" @click.stop.prevent="open('/help/unavailable')">Why?</b-button>
                </span>
              </template>
            </template>
