<template>
  <div class="grid grid-cols-2 gap-8" style="grid-template-columns: 300px 1fr">
    <div class="h-full flex flex-col justify-start items-stretch gap-2">
      ...
      <b-button v-if="labCsvState.status === LabCsvStatus.UPLOADED_COAS" :disabled="hasErrors" variant="success"
        @click="submit()">SUBMIT</b-button>

      <b-button v-if="labCsvState.status === LabCsvStatus.SELECTED_COAS" :disabled="hasErrors" variant="success"
        @click="uploadCOAs()">UPLOAD COA PDFs TO METRC</b-button>
      ...
      <b-button v-if="labCsvState.status !== LabCsvStatus.INITIAL" @click="reset()"
        variant="outline-dark">RESET</b-button>
      ...
      <template v-if="showOutputTable">
        <div class="grid grid-cols-3 gap-2" style="grid-template-columns: 1fr 80px 1fr">
          <fragment v-for="[idx, richPackage] of richPackageLabData.entries()" v-bind:key="idx">
            ...
            <template v-else>
              <canonical-package-card v-if="richPackage.pkg" :pkg="richPackage.pkg"></canonical-package-card>
            </template>
            ...
            <template v-else>
              <template v-if="richPackage.file.metrcFileId">
                <div class="rounded-md border flex flex-row items-center justify-center gap-2 text-lg text-green-600"
