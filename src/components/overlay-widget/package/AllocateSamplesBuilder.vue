      ...
      try {
        const csvData = buildCsvDataOrError([
          ...
          },
          {
            isVector: true,
            data: (this.selectedSampleAllocations as ISampleAllocation[]).map(
              (x) => this.$data.unitsOfMeasure.find(
          ...
          {
            isVector: false,
            data: this.$data.adjustmentReasons.find((x: IAdjustPackageReason) => x.Name.includes('Trade Sample'))!.Name,
          },
          {
            isVector: true,
            data: (this.selectedSampleAllocations as ISampleAllocation[]).map(
              (x) => `${x.employee.FullName} ${x.employee.License.Number} ${x.adjustmentQuantity} ${x.pkg.UnitOfMeasureAbbreviation}`,
            ),
          },
          {
            isVector: false,
