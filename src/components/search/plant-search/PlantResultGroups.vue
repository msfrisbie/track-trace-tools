...
export default Vue.extend({
  ...
  computed: {
    ...
    labelPlants(): IIndexedPlantData[] {
      const plants = this.plants.filter((plantData) => plantData.Label.includes(store.state.search.queryString));

      return plants;
    },
    strainNamePlants(): IIndexedPlantData[] {
      const plants = this.plants.filter((plantData) => plantData.StrainName?.toUpperCase().includes(store.state.search.queryString.toUpperCase()));

      return plants;
    },
    locationNamePlants(): IIndexedPlantData[] {
      const plants = this.plants.filter((plantData) => plantData.LocationName?.toUpperCase().includes(store.state.search.queryString.toUpperCase()));
