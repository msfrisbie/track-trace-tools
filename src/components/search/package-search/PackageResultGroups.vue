...
export default Vue.extend({
  ...
  computed: {
    ...
    labelPackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.Label.includes(store.state.search.queryString));

      return packages;
    },
    sourceHarvestNamePackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.SourceHarvestNames?.toUpperCase().includes(
    ...
    sourcePackageLabelPackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.SourcePackageLabels?.toUpperCase().includes(
        store.state.search.queryString.toUpperCase(),
      ));
    ...
    productionBatchNumberPackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.ProductionBatchNumber?.toUpperCase().includes(
        store.state.search.queryString.toUpperCase(),
      ));
    ...
    sourceProductionBatchNumbersPackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.SourceProductionBatchNumbers?.toUpperCase().includes(
        store.state.search.queryString.toUpperCase(),
      ));
    ...
    itemNamePackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.Item?.Name?.toUpperCase().includes(store.state.search.queryString.toUpperCase()));

      return packages;
    },
    itemStrainNamePackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.Item.StrainName?.toUpperCase().includes(
        store.state.search.queryString.toUpperCase(),
      ));
    ...
    locationNamePackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.LocationName?.toUpperCase().includes(
        store.state.search.queryString.toUpperCase(),
      ));
    ...
    itemProductCategoryNamePackages(): IIndexedPackageData[] {
      const packages = this.packages.filter((packageData) => packageData.Item.ProductCategoryName?.toUpperCase().includes(
        store.state.search.queryString.toUpperCase(),
      ));
