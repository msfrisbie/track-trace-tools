<template>
  <div class="flex flex-col items-center text-start gap-8">
    <div id="changelog" class="flex flex-col items-center px-6">
      <div class="flex flex-col items-stretch max-w-md gap-16">
        <div class="max-w-lg flex flex-col gap-4">
          <p class="text-2xl ttt-purple">v0.56.0</p>

          <ul class="list-disc">
            <li>Search v2</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.54.1</p>

          <ul class="list-disc">
            <li>Updated search interface</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.45.0</p>

          <ul class="list-disc">
            <li>Finished Metrc Explorer</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.44.0</p>

          <ul class="list-disc">
            <li>Fixes for reports</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.39.0</p>

          <ul class="list-disc">
            <li>New quick scripts and settings</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.37.0</p>

          <ul class="list-disc">
            <li>Fixes to transfer tool and metrc repeaterData interface</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.36.0</p>

          <ul class="list-disc">
            <li>Updates to transfer tool</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.35.0</p>

          <ul class="list-disc">
            <li>Add search survey and demo video</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.34.0</p>

          <ul class="list-disc">
            <li>Cosmetic changes</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.32.0</p>

          <ul class="list-disc">
            <li>Allow for settings to be persisted in Chrome storage</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.30.1</p>

          <ul class="list-disc">
            <li>Fixes for scan sheets</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.29.0</p>

          <ul class="list-disc">
            <li>Scan sheeets launch for T3+</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.28.0</p>

          <ul class="list-disc">
            <li>Report enhancements</li>
            <li>Metrc Explorer beta launch</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.27.0</p>

          <ul class="list-disc">
            <li>T3+ beta</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.26.0</p>

          <ul class="list-disc">
            <li>Improvements to data load</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.25.0</p>

          <ul class="list-disc">
            <li>Cosmetic updates</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.24.0</p>

          <ul class="list-disc">
            <li>Additional reports</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.23.0</p>

          <ul class="list-disc">
            <li>Enhancements for reports</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.22.1</p>

          <ul class="list-disc">
            <li>Fixes for unpack plant batch packages</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.22.0</p>

          <ul class="list-disc">
            <li>Fix OAuth login</li>
            <li>Add free export types</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.21.2</p>

          <ul class="list-disc">
            <li>Fix item picker</li>
          </ul>
          <p class="text-2xl ttt-purple">v0.21.1</p>

          <ul class="list-disc">
            <li>Add Google Sheets report tool</li>
          </ul>
          <p class="text-2xl ttt-purple">v0.20.0</p>

          <ul class="list-disc">
            <li>Fix for plant batch retag</li>
            <li>Add settings option to change default date submit format</li>
          </ul>
          <p class="text-2xl ttt-purple">v0.19.0</p>

          <ul class="list-disc">
            <li>Added bulk package content quick script</li>
          </ul>
          <p class="text-2xl ttt-purple">v0.18.0</p>

          <ul class="list-disc">
            <li>Updates to tag picker</li>
            <li>Fixes to mothering date picker</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.17.0</p>

          <ul class="list-disc">
            <li>Added support for medical tags</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.14.1</p>

          <ul class="list-disc">
            <li>Fix for plant picker</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.14.0</p>

          <ul class="list-disc">
            <li>Infrastructure for Google Sheets integration</li>
            <li>Updates to quick scripts</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.13.0</p>

          <ul class="list-disc">
            <li>Updates to quick scripts</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.12.0</p>

          <ul class="list-disc">
            <li>Added support for vegetative plants in tools</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.11.0</p>

          <ul class="list-disc">
            <li>Added support for auto-opening any tab</li>
            <li>Added Quick Scripts</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.10.0</p>

          <ul class="list-disc">
            <li>Private feature updates</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.9.0</p>

          <ul class="list-disc">
            <li>Private feature updates</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.8.0</p>

          <ul class="list-disc">
            <li>Add public interface for client licenses</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.7.0</p>

          <ul class="list-disc">
            <li>Custom snowflake features</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.6.0</p>

          <ul class="list-disc">
            <li>Dark mode beta</li>
            <li>Updated snowflakes controls</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.5.1</p>

          <ul class="list-disc">
            <li>Fix timezone handling of date selection</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.5.0</p>

          <ul class="list-disc">
            <li>Unify tag picker interface</li>
            <li>Add replace tags tools for plants and plant batches</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.4.0</p>

          <ul class="list-disc">
            <li>Remove popup window</li>
            <li>Tag range selection</li>
            <li>Fix plural noun in submit view</li>
            <li>Enable lab test views on all origins</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.3.0</p>

          <ul class="list-disc">
            <li>Update plant filter</li>
            <li>Allow for uniform distribution of weight for batch operations</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.2.4</p>

          <ul class="list-disc">
            <li>Adjust item picker formatting</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.2.3</p>

          <ul class="list-disc">
            <li>Adjust promo slider styling</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.2.2</p>

          <ul class="list-disc">
            <li>Additional T3 promo slider content</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.2.1</p>

          <ul class="list-disc">
            <li>Fix T3 promo slider</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.2.0</p>

          <ul class="list-disc">
            <li>Add T3 promo slider</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.1.0</p>

          <ul class="list-disc">
            <li>Add T3 dashboard</li>
            <li>Fix font loading</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.0.3</p>

          <ul class="list-disc">
            <li>Remove transfer origin check</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.0.2</p>

          <ul class="list-disc">
            <li>Fix unit tests</li>
            <li>Update CircleCI</li>
            <li>Remove access control list blockers</li>
          </ul>

          <p class="text-2xl ttt-purple">v0.0.1</p>

          <ul class="list-disc">
            <li>Initial release</li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import router from '@/router/index';
import store from '@/store/page-overlay/index';
import Vue from 'vue';
import { mapState } from 'vuex';

export default Vue.extend({
  name: 'Changelog',
  store,
  router,
  props: {},
  components: {},
  computed: {
    ...mapState([]),
  },
  data() {
    return {};
  },
  methods: {},
  async created() {},
  async mounted() {},
});
</script>

<style type="text/scss" lang="scss" scoped></style>
